...
def get_emr_cluster_id_by_name(cluster_name: str) -> str:
    """
    Get EMR cluster ID by cluster name (only returns RUNNING or WAITING clusters)
    """
    try:
        # List clusters that meet the criteria
        response = emr_client.list_clusters(
            ClusterStates=['RUNNING', 'WAITING'],  # Only find active clusters
            MaxResults=10
        )
        
        # Iterate through cluster list, match by name
        for cluster in response['Clusters']:
            if cluster['Name'] == cluster_name:
                return cluster['Id']
        
        # No matching cluster found
        print(f"Active EMR cluster with name {cluster_name} not found")
        return None
    
    except Exception as e:
        print(f"Failed to get EMR cluster ID: {e}")
        return None

def submit_emr_spark_job(task_id: str):
    # Dynamically get cluster ID first
    cluster_id = get_emr_cluster_id_by_name(EMR_CLUSTER_NAME)
    if not cluster_id:
        return None
            ...
            Steps=[
                {
                    ...
                    'HadoopJarStep': {
                        ...
                        'Args': [
                            'spark-submit',
                            '--deploy-mode', 'client',
                            f'{S3_BUCKET}/crime_classification.py'  # Script path on S3
                        ]
                    }
                }
            ]
        )
        print(f"Task submitted successfully, Step ID: {response['StepIds'][0]}")
        return response['StepIds'][0]
    
    except emr_client.exceptions.ValidationException as e:
        print(f"EMR submission failed (parameter validation error): {e}")
        return None
    except emr_client.exceptions.ClientError as e:
        error_code = e.response['Error']['Code']
        error_msg = e.response['Error']['Message']
        print(f"EMR client error ({error_code}): {error_msg}")
        return None
    except Exception as e:
        print(f"EMR submission failed: {e}")
        return None
