...
def get_emr_cluster_id_by_name(cluster_name: str) -> str:
    """
    通过 EMR 集群名称获取集群 ID（仅返回 RUNNING 或 WAITING 状态的集群）
    """
    try:
        # 列出符合条件的集群
        response = emr_client.list_clusters(
            ClusterStates=['RUNNING', 'WAITING'],  # 只找活跃的集群
            MaxResults=10
        )
        ...
        # 未找到对应集群
        print(f"未找到名称为 {cluster_name} 的活跃 EMR 集群")
        return None
    
    except Exception as e:
        print(f"获取 EMR 集群 ID 失败: {e}")
        return None
